...
def test_injection_self_heal_stays_alive_in_degraded_mode_without_qianniu_restart():
...
def test_post_update_recovery_is_version_bound_but_non_destructive():
    repair = read("src/Bot/Update/BotUpdateStartupConnection.Fast.cs")
    ...
    assert "不自动重启千牛" in repair
    assert "不操作登录界面" in repair
