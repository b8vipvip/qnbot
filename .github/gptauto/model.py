...
@dataclass
class Event:
    at:str; state:str; reason:str; kind:str="state"; gate:str=""; status:str=""; evidence:str=""
@dataclass
class Criterion:
...
class Task:
    ...
    def save(self,path):
        path=Path(path);path.parent.mkdir(parents=True,exist_ok=True);path.write_text(json.dumps(self.to_dict(),indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
    @classmethod
