...
def gate(task,name,status,evidence=""):
    ...
    step.status=status;step.evidence=evidence;task.record(f"{name.value}={status.value}: {evidence}".rstrip())
    if status==GateStatus.FAILED:
        task.repair_attempts+=1
        if task.repair_attempts>task.max_repair_attempts:task.state=State.BLOCKED;task.record("repair budget exhausted")
    return task
...
def criterion(task,index,status,evidence=""):
    ...
    c=task.definition_of_done[index];c.status=status;c.evidence=evidence;task.record(f"DoD[{index}]={status.value}: {evidence}".rstrip());return task
def finish(task):
    ...
    task.state=State.DONE;task.record("Definition of Done satisfied");return task
def block(task,reason):task.state=State.BLOCKED;task.record(reason);return task
